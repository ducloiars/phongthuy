...
def get_brand_voice_from_db():
    # Resolve the path to brain.db (checking root of skill, workspace root, and current dir)
    db_candidates = [
        '/app/brain.db',
        os.path.join(os.path.dirname(__file__), '../../My_Brain/brain.db'),
        os.path.join(os.path.dirname(__file__), '../../my-brain/brain.db'),
        os.path.join(os.path.dirname(__file__), '../brain.db'),
        os.path.join(os.path.dirname(__file__), '../../brain.db'),
        'brain.db'
    ]
    
    # Hardcoded fallbacks including style examples (matching brand-voice.md)
    fallback_voice = {
...
Mà là sau tất cả, họ đã sống như thế nào với người khác.
 #suyngam #songtichcuc #baihoccuocsong #longtot""",
...
#songtichcuc #baihoccuocsong #suyngam #songtinhte"""
    }
    ...
    try:
        ...
        for row in rows:
            ...
            if "Tone" in title:
                ...
                brand_voice[title] = content
                
        # Ensure we have examples in the returned dict, otherwise use fallback ones
        for k, v in fallback_voice.items():
            if k not in brand_voice:
                brand_voice[k] = v
...
def generate_caption(topic, mode, angle):
    ...
    # Extract style examples to feed into system prompt
    style_examples_text = ""
    for key, val in brand_voice.items():
        if "Ví dụ" in key or "ví dụ" in key:
...
DƯỚI ĐÂY LÀ CÁC VÍ DỤ VỀ VĂN PHONG TIÊU BIỂU (BẮT BUỘC PHẢI BẮT CHƯỚC THEO ĐÚNG CẤU TRÚC NGẮT DÒNG VÀ CÁCH DÙNG DẤU BA CHẤM, EMOJI):
{style_examples_text}
    ...
    while attempts < max_attempts:
        try:
            ...
            if attempts < max_attempts:
                ...
                print("⚠️ OpenAI API failed. Generating high-quality local mock caption based on Brand Voice and templates...", file=sys.stderr)
